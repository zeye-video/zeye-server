...
const youtubeLinkParser = require('./youtubeLinkParser');
...
class Room extends EventEmitter
{
	...
	{
		// THERE SHOULD BE AUTH CHECK WITH NODE_SESSION_LARAVEL

		switch (request.method)
		{
			case 'chatMessage' : {
				this._getJoinedPeers().forEach((peer) => {
					...
					text = text.split(' ').map((word) => {
						const isLink = word.match(expression)

						if (isLink !== null) {
							if (isImageUrl(word)) {
								return `<a href="${word}" rel="noopener noreferrer" target="_blank">
											<img src="${word}" alt="${word}" class="chat-image">
										</a>`
							}

							if (youtubeLinkParser(word)) {
								return `<iframe width="200" 
												height="150" 
												src="https://www.youtube.com/embed/${youtubeLinkParser(word)}" 
												frameborder="0" 
					...
					peer.notify('chatMessage', message)
				})
				accept()
				break;
			}
